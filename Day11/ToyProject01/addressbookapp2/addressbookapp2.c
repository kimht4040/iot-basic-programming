#define _CRT_SECURE_NO_WARNINGS

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define MAX_CONTRACTS 100
#define NAME_LEN 31
#define PHONE_LEN 21
#define EMAIL_LEN 65
#define ADDR_LEN 129
#define MEMO_LEN 129

typedef struct _contact {
	char name[NAME_LEN];
	char phone[PHONE_LEN];
	char email[EMAIL_LEN];
	char address[ADDR_LEN];
	char memo[MEMO_LEN];
} Contact;

static Contact contacts[MAX_CONTRACTS];
static int count = 0;
static void read_line(char* buf, int size);
static void add_contact();
static void print_menu();
static int read_menu();
static void list_contacts();




int main() {
	int choice;


	while (1) {
		print_menu();
		choice = read_menu();
		switch (choice) {
		case 1:
			//printf("연락처 추가\n");
			add_contact();
			break;
		case 2:
			//printf("연락처 목록\n");
			list_contacts();
			break;
		case 3:
			printf("연락처 수정\n");
			break;
		case 4:
			printf("연락처 삭제\n");
			break;
		case 5:
			printf("연락처 검색\n");
			break;
		case 6:
			printf("종료\n");
			exit(1);
			break;
		default:
			printf("1 ~ 6사이에 선택\n");
		}


	}



	return 0;
}
static void add_contact() {
	if (count >= MAX_CONTRACTS) {
		puts("주소록이 가득찼습니다.");
		return;
	}
	puts("새 연락처 입력");
	printf("이름 : ");
	read_line(contacts[count].name, NAME_LEN);
	printf("전화 : ");
	read_line(contacts[count].phone, NAME_LEN);
	printf("주소 : ");
	read_line(contacts[count].address, NAME_LEN);
	printf("이메일 : ");
	read_line(contacts[count].email, NAME_LEN);
	printf("메모 : ");
	read_line(contacts[count].memo, NAME_LEN);
	count++;
	puts("추가완료");
}
static void read_line(char* buf, int size) {
	if (fgets(buf, size, stdin) == NULL) {
		buf[0] = '\0';
		return;
	}
	//strcspn(buf, "\n") => 버퍼의 위치에서 공백 위치 값을 리턴
	buf[strcspn(buf, "\n")] = '\0';

}
static void print_menu() {
	puts("==================================================");
	puts("              주소록 프로그램 (STEP 1)            ");
	puts("==================================================");
	printf("기능\n1.추가\n2.목록\n3.수정\n4.삭제\n5.검색\n6.종료\n");
}
static int read_menu() {
	int choice, ch;
	printf("선택 > ");
	if (scanf("%d", &choice) != 1) {

		while ((ch = getchar()) != '\n' && ch != EOF);
		return -1;
	}
	while ((ch = getchar()) != '\n' && ch != EOF);

	return choice;
}


static void list_contacts() {
	if (count == 0) {
		printf("저장된 연락처 없음\n");
		return;
	}
	puts("연락처 목록");
	puts("--------------------------------------------------------------------");
	printf("개수: %d\n", count);
	printf("%4s | %10s | %15s | %25s | %30s\n",
		"No", "Name", "Phone", "Email", "Address");
	puts("--------------------------------------------------------------------");
	for (int i = 0; i < count; i++) {
		printf("%4d | %10s | %15s | %25s | %30s\n",
			i+1 , contacts[i].name, contacts[i].phone, contacts[i].email, contacts[i].address);
	}

}